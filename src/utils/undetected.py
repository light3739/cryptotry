...
class UndetectedSetup:
    ...
    async def initialize_driver(self):
        try:
            ...
            logger.debug(f"Browser started: {self.browser}")

            logger.debug("Getting main tab...")
            self.main_tab = self.browser.main_tab
            logger.debug(f"Main tab obtained: {self.main_tab}")

            if self.main_tab is None:
                raise ValueError("Failed to get main tab")
